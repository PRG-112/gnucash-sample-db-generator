...
I never could find a proper GnuCash sample database to play with. Now you can generate your own one with this tool in no time. It's not perfect, but good enough. A sample database (1-JAN-2023 -> 31-DEC-2026) + premade-settings/reports (GCM) included. (see: EXTRAS)
...
By default, the generator is set to USD and starts as of 1-JAN-2023 (WARNING: if your default currency is different, reports won't show anything until you change the commodity in the report's settings). If you're looking for something different, edit the generator's code lines:
...
- copy the GCM file to "GNC_DATA_HOME\books" directory (run GnuCash and click: HELP -> ABOUT -> GNC_CONFIG_HOME to see where it's at) then open the sample database with the same name as the file. (remember that the provided sample DB is tied with the GCM via specific UUID)
